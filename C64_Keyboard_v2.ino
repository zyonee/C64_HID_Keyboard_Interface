...
35,42,59,47,133,61,211,210,//Â£ * ; / RSHFT = Pi Home
...
35,42,93,63,133,61,214,210,//Â£ * ; ? RSHFT = Pi Home
...
209,93,39,47,133,92,212,210,//Â£ * ; / RSHFT = Pi Home
...
void loop() // main keyboard scanning loop
{
  ...
  for (outPin=2;outPin<10; outPin++) // scan through all rows
    ...
    for (i=0; i<8; i++) // scan through columns
      {
      keyPos=i+((outPin-2)*8); // calculate character map position
      if (!windowsShift) inChar=keyMap[keyPos+shift]; // work out which key it is from the map and shift if needed
       else inChar=keyMap[keyPos+128];  // use "windows" keymap where shift is passed through
   ...
   if ((millis()-lastDebounceTime[keyPos])>debounceDelay) // debounce for each key individually
   {
      if (digitalread==1 && keyDown[keyPos]==0) // if a key is pressed and wasn't already down
        ...
        if ((keyPos!=11&&keyPos!=52)||windowsShift==1)// is it not-shift or in windows mode?
        ...
        else { lastDebounceTime[keyPos]=millis(); shift=64; } // reset keybounce delay and mark as shift press
        }
      if (digitalread==0 && keyDown[keyPos]!=0) // key is up and a character is stored in the keydown position
        {
        if ((keyPos!=11&&keyPos!=52)||windowsShift==1) // not-shift or windows mode
          {
  ...
  digitalWrite(outPin,HIGH); // set output back to high
 }
