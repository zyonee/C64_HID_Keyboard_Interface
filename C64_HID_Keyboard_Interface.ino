...
// Startup mode switching works as follows:
// If Windows mode is the default
//   Hold 1 at startup for C64 mode with US Layout
//   Hold 2 at startup for C64 mode with EU Layout
//
// If C64 mode is the default
//   Hold 1 at startup for Windows mode
//   Hold 2 at startup for C64 mode with EU Layout
...
// detect if '1' or '2' are held on power up to swap modes and keymaps
  if (DefaultKBMode==1)                                        // if the default mode is C64 mode
  {
    ...
    {
      windowsShift=1;                                          // if pressed, set Windows mode
    } else {
      USKeyboard=1;                                            // otherwise set US layout

      digitalWrite(2,LOW);                                     // need to reset pin 2 before each read
      if (!digitalRead(A3))                                    // read key '2'
      {
        USKeyboard=0;                                          // if pressed, set EU layout
      }
    }
  }
  ...
  if (DefaultKBMode==0)                                        // if the default mode is C64 mode
  {
    ...
    if (!digitalRead(10))                                      // read key '1'
    {
      windowsShift=0;                                          // if pressed set C64 mode
      USKeyboard=1;                                            // and set US layout
    } else {
      digitalWrite(2,LOW);                                     // need to reset pin 2 before each read
      if (!digitalRead(A3))                                    // read key '2'
      {
          windowsShift=0;                                      // if pressed, set C64 mode
          USKeyboard=0;                                        // and set EU layout
      }
    }
  }
}
